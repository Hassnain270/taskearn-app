...
export default function AdminAllWithdrawalsScreen({ navigation }) {
  ...
  useEffect(() => {
    const checkAccessAndLoad = async () => {
      try {
        ...
        const getAllWithdrawals = httpsCallable(functions, 'adminGetAllWithdrawals');
        const res = await getAllWithdrawals();
        setWithdrawals((res.data && res.data.withdrawals) || []);
      } catch (err) {
  ...
  const sections = useMemo(() => {
    ...
    filteredWithdrawals.forEach((w) => {
      const label = w.date ? getDayLabel(w.date) : 'Unknown Date';
      if (!grouped[label]) {
        grouped[label] = [];
        order.push(label);
      }
  ...
  const renderItem = ({ item }) => (
    <View style={currentStyles.itemCard}>
      ...
      <View style={styles.itemMiddle}>
        <Text style={currentStyles.itemUsername}>{item.username}</Text>
        <Text style={styles.itemSubtext}>{formatTime(item.date)}</Text>
      </View>
    ...
    return (
      <SafeAreaView style={currentStyles.container} edges={['top']}>
        ...
        <View style={currentStyles.header}>
          ...
          <Text style={currentStyles.headerTitle}>All Withdrawals</Text>
          <View style={{ width: 36 }} />
        </View>
...
const lightStyles = StyleSheet.create({
  ...
  itemUsername: { fontSize: 13, fontWeight: '700', color: '#1E293B' },
...
const darkStyles = StyleSheet.create({
  ...
  itemUsername: { fontSize: 13, fontWeight: '700', color: '#FFFFFF' },
...
const styles = StyleSheet.create({
  ...
  avatarCircle: { width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(239,68,68,0.12)', justifyContent: 'center', alignItems: 'center', marginRight: 10 },
  itemMiddle: { flex: 1 },
  itemSubtext: { fontSize: 10, fontWeight: '500', color: '#94A3B8', marginTop: 2 },
  itemAmount: { fontSize: 14, fontWeight: '800', color: '#EF4444' },
