...
export default function DepositScreen({ navigation }) {
  ...
  // Listens to the server-side deposit record. The scheduled Cloud Function
  // checks the blockchain and updates this record automatically, even if
  // this screen (or the app) is closed in the meantime. If the user is still
  // here when it happens, they see a live confirmation.
  useEffect(() => {
    if (!depositId) return;

    const depositRef = doc(db, 'depositAddresses', depositId);
    const unsubscribe = onSnapshot(depositRef, (snap) => {
      ...
      if (data.status === 'confirmed') {
        Alert.alert(
          "Deposit Confirmed!",
          `Successfully received deposit via ${selectedNetwork}. $${Number(data.confirmedAmount).toFixed(2)} USDT has been added to your balance.`,
    ...
    try {
      ...
      if (user?.uid) {
        ...
        if (selectedNetwork === 'TRC20') {
          result = await generateDepositAddress(depositAmount);
        } else {
          result = await generateBEP20DepositAddress(depositAmount);
        }
    ...
    <SafeAreaView style={currentStyles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.feeHintBox}>
          ...
          <Text style={styles.feeHintText}>{NETWORK_FEE_HINT[selectedNetwork]}</Text>
        </View>
          ...
          <View style={currentStyles.addressSectionBox}>
            ...
            <View style={styles.waitingStatusRow}>
              ...
              <Text style={styles.statusText}>
                Deposit is being verified automatically in the background. You can safely close this screen or the app — your balance will update once confirmed.
              </Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  feeHintBox: { flexDirection: 'row', gap: 6, marginTop: 8, paddingHorizontal: 2 },
  feeHintText: { flex: 1, color: '#64748B', fontSize: 11, lineHeight: 15 },
  ...
  waitingStatusRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, marginTop: 20 },
  spinnerPlaceholder: { width: 14, height: 14, borderRadius: 7, borderWidth: 2, borderColor: '#3B82F6', borderTopColor: 'transparent', marginTop: 2 },
  statusText: { color: '#64748B', fontSize: 12, flex: 1, lineHeight: 17 },
